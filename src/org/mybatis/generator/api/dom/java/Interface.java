...
public class Interface extends JavaElement implements CompilationUnit {
	...
	//yws update start  泛型的类型
	private List<FullyQualiExtendJavaType> genericsTypes;
	
	public List<FullyQualiExtendJavaType> getGenericsTypes() {
		return genericsTypes;
	}
	public void setGenericsTypes(List<FullyQualiExtendJavaType> genericsTypes) {
	...
	public String getFormattedContent() {
		...
		List<FullyQualiExtendJavaType> genericsTypes = getGenericsTypes();
		if(genericsTypes != null && !genericsTypes.isEmpty()){
			sb.append("<");
			int size = genericsTypes.size();
			for(int i=0; i < size ; i++){
				sb.append(genericsTypes.get(i).getExtendClassName());
				if(i != (size -1)){
					sb.append(",");
				}
			}
